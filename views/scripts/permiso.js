...
function edit(i){ //funcion para guardary editar los datos
	...
	$.ajax({
		url: "../ajax/permiso.php?action=edit", //url donde voy a enviar los data
		type: "POST",
		data: formData, //en data paso todos los datos mediante la variable formData (que captura todos los datos del formulario)
		contentType: false,
		processData: false,
...
function mostrar(codpermiso){
	$.post("../ajax/permiso.php?action=mostrar",{codpermiso : codpermiso}, function(data,status){

		data = JSON.parse(data);
		mostrarFormEdit(true); //se visualiza el formulario de regitro para ver os datos

		$("#codpermiso_edit").val(data.CodPermiso);
		$("#nombre_edit").val(data.NombrePermiso);
		$("#detalle_edit").val(data.Detalle);
		//$("#id_user").val(data.updated_by);
		$("#codpermiso_id_edit").val(data.CodPermiso);
	}) 	
}

function disable(codpermiso){
	bootbox.confirm("<div class='alert alert-warning alert-dismissable'>¿Desea desactivar el acceso?</div>", function(result){
		if (result) {
			$.post("../ajax/permiso.php?action=disable",{codpermiso : codpermiso}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			})
		}
