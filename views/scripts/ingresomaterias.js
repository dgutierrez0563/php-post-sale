...
function init(){
	...
	$.post("../ajax/ingresomaterias.php?action=listarProveedor", function(r) { //el parametro 'r' son las opcioes que nos esta devolviendo la funcion ajax de articulo.php
		$("#codproveedor").html(r);
...
function showAll(){ //funcion para mostrar el listado de datos
	tabla = $('#tb_listado').dataTable({
		...
		"ajax": {
			url: '../ajax/ingresomaterias.php?action=showAll',
			type: "get",
			dataType: "json",
			error: function(e){
...
function listarArticulos(){ //funcion para mostrar el listado de datos
	tabla = $('#tb_listadoarticulosmodal').dataTable({
		...
		"ajax": {
			url: '../ajax/ingresomaterias.php?action=listarArticulos',
			type: "get",
			dataType: "json",
			error: function(e){
				console.log(e.responseText);
...
function save_edit(e){ //funcion para guardary editar los datos
	...
	$.ajax({
		url: "../ajax/ingresomaterias.php?action=save_edit", //url donde voy a enviar los data
		type: "POST",
...
function mostrar(codingreso){
	$.post("../ajax/ingresomaterias.php?action=mostrar",{codingreso : codingreso}, function(data,status){
		...
		// $("#proveedor_aux").show();
		// $("#fechahora_aux").show();
		// $("#tipo_comprobante_aux").show();
		// $("#seriecomprobante_aux").show();
		// $("#numerocomprobante_aux").show();
		// $("#impuesto_aux").show();

		// $("#proveedor_aux").val(data.NombreProveedor);
		// $("#fechahora_aux").val(data.Fecha);
		// $("#tipo_comprobante_aux").val(data.TipoComprobante);
		// $("#seriecomprobante_aux").val(data.SerieComprobante);
		// $("#numerocomprobante_aux").val(NumeroComprobante);
		// $("#impuesto_aux").val(data.Impuesto);

		// $("#codproveedor").hide();
		// $("#fechahora").hide();
		// $("#tipo_comprobante").hide();
		// $("#seriecomprobante").hide();
	...
	$.post("../ajax/ingresomaterias.php?action=listarDetalleIngresos&id="+codingreso, function(e){
...
function agregardetallesingreso(codarticulo,articulo){
	...
	if (codarticulo!="") {
		...
		var fila = '<tr class="filas" id="fila'+count+'">'+
			...
			'<td><input type="number" name="preciocomp[]" id="preciocomp[]" value="'+preciocomp+'"></td>'+
			'<td><input type="number" name="preciovent[]" value="'+preciovent+'"></td>'+
