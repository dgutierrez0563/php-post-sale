...
function mostrarForm_edit(flag){ //funcion del boton para llamar el modal para registrar categorias nuevas
	limpiar();
	if(flag){
		$("#listado_registros").hide(); //oculta la tabla del listado de datos
...
function showAll(){ //funcion para mostrar el listado de datos
	tabla = $('#tb_listado').dataTable({
		...
		"ajax": {
			url: '../ajax/departamento.php?action=showAll',
			type: "get",
			dataType: "json",
			error: function(e){
...
function save_edit(e){ //funcion para guardary editar los datos
	...
	$.ajax({
		url: "../ajax/departamento.php?action=save_edit", //url donde voy a enviar los data
		type: "POST",
...
function edit(i){ //funcion para guardary editar los datos
	...
	$.ajax({
		url: "../ajax/departamento.php?action=edit", //url donde voy a enviar los data
		type: "POST",
		data: formData, //en data paso todos los datos mediante la variable formData (que captura todos los datos del formulario)
		contentType: false,
		processData: false,
		...
		success: function(datos){ // en datos_alerta retorno desde categoria.php en ajax los mensajes de echo "", accion realizada
			bootbox.alert(datos); //envio al alert que aparecera en el index categoria el msj de accion realizada
			mostrarForm_edit(false); //cada vez que se ejecuta la accion guardar o editar, de inmediato oculto el div modal con false
			tabla.ajax.reload(); //luego de cerrar u ocultar el div modal recargo la tabla de informacion de categorias
			//esta variable u objeto tabla, hace referencia a la variable tabla en la funcion javascript de showAll() que captura
			//toda la informacion de categorias.
		}
...
function mostrar(coddepartamento){
	$.post("../ajax/departamento.php?action=mostrar",{coddepartamento : coddepartamento}, function(data,status){

		data = JSON.parse(data);
		mostrarForm_edit(true); //se visualiza el formulario de regitro para ver os datos

		$("#coddepartamento_edit").val(data.CodDepartamento);
		$("#nombre_edit").val(data.NombreDepartamento);
		//$("#detalle_edit").val(data.Detalle);
		$("#coddepartamento_id_edit").val(data.CodDepartamento);
	}) 	
}

function disable(coddepartamento){
	bootbox.confirm("<div class='alert alert-warning alert-dismissable'>¿Desea desactivar el departamento?</div>", function(result){
		if (result) {
			$.post("../ajax/departamento.php?action=disable",{coddepartamento : coddepartamento}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			})
		}
...
function enable(coddepartamento){
	bootbox.confirm("<div class='alert alert-warning alert-dismissable'>¿Desea activar el departamento?</div>", function(result){
		if (result) {
			$.post("../ajax/departamento.php?action=enable",{coddepartamento : coddepartamento}, function(e){
				bootbox.alert(e);
				tabla.ajax.reload();
			})
		}
...
function detalle(coddepartamento){
	$.post("../ajax/departamento.php?action=detalle",{coddepartamento : coddepartamento}, function(data,status){

		$("#listado_registros").hide();
		data = JSON.parse(data);
		mostrarForm_detalle(true); //se visualiza el formulario de regitro para ver os datos

		$("#coddepartamento_p").val(data.CodDepartamento);
		$("#nombre_p").val(data.NombreDepartamento);
		...
		$("#coddepartamento_id_p").val(data.CodDepartamento);
	}) 	
}
