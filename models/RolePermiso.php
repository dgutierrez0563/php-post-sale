	...
	class RolePermiso
	{
		...
		*/
		...
		public function existeRolePermisoID($id_role_asignacion,$id_permiso){
			$sql = "SELECT CodRolePermiso FROM rolepermiso WHERE CodRole='$id_role_asignacion' AND CodPermiso='$id_permiso'";
			return existeRolePermiso($sql);
		}

		public function viewRolePermiso($id_role){
			$sql = "SELECT r.Nombre, p.CodPermiso,p.NombrePermiso FROM rolepermiso rp INNER JOIN role r ON rp.CodRole=r.CodRole
