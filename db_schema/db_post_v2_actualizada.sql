...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 13-03-2019 a las 07:10:00
...
('10', 'Needs it Tech', 'Cedula Juridica', '3103201156', 'Sta. Monica, California', '22124563', 'dashboard@yahoo.com', 1, '1', '1', '2018-12-25 16:08:15', '2019-03-13 05:54:11'),
...
--
-- Estructura de tabla para la tabla `cajausuario`
--
CREATE TABLE `cajausuario` (
  `IDCajausuario` varchar(8) NOT NULL,
  `CodCaja` varchar(4) NOT NULL,
...
CREATE TABLE ventas(
  ...
  TipoComprobante varchar(5) DEFAULT NULL,
  SerieComprobante varchar(5) DEFAULT NULL,
  NumeroComprobante int(11) NOT NULL,
  FechaComprobante datetime NOT NULL,
  Inpuesto decimal(11,2) NOT NULL,
  TotalCompra decimal(11,2) NOT NULL,
...
ALTER TABLE `detalleventas`
  MODIFY `IDDetalleVenta` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=1;
...
ALTER TABLE `cliente`
  ADD CONSTRAINT `cliente_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `usuario` (`CodUsuario`),
  ADD CONSTRAINT `cliente_ibfk_2` FOREIGN KEY (`updated_by`) REFERENCES `usuario` (`CodUsuario`);
...
ALTER TABLE `ventas`
  ADD CONSTRAINT `ventas_ibfk_1` FOREIGN KEY (`created_by`) REFERENCES `usuario` (`CodUsuario`),
  ADD CONSTRAINT `ventas_ibfk_2` FOREIGN KEY (`updated_by`) REFERENCES `usuario` (`CodUsuario`);

ALTER TABLE `detalleventas`
  ADD CONSTRAINT `detalleventas_ibfk_1` FOREIGN KEY (`IDVenta`) REFERENCES `ventas` (`IDVenta`),
  ADD CONSTRAINT `detalleventas_ibfk_2` FOREIGN KEY (`CodArticulo`) REFERENCES `articulo` (`CodArticulo`),
  ADD CONSTRAINT `detalleventas_ibfk_3` FOREIGN KEY (`created_by`) REFERENCES `usuario` (`CodUsuario`),
  ADD CONSTRAINT `detalleventas_ibfk_4` FOREIGN KEY (`updated_by`) REFERENCES `usuario` (`CodUsuario`);
--
-- Disparadores `detalleingreso` ingresa stock al producto en articulos cuando se ingresa una nueva entrada de inventario
--
DELIMITER $$
CREATE TRIGGER `tr_updateStockIngresoMaterias` 
AFTER INSERT ON `detalleingreso` FOR EACH ROW 
BEGIN
    UPDATE articulo SET Stock = Stock + NEW.Cantidad
    WHERE articulo.CodArticulo = NEW.CodArticulo;
END
$$
DELIMITER ;

--
-- Disparadores `ventas` rebajar stock al producto en articulos cuando se ingresa una nueva venta
--
DELIMITER //
