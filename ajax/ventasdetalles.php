...
<?php
	...
	require_once "../models/VentasDetalles.php";

	$ventas = new VentasDetalles();
	...
	switch ($_GET["action"]) {
		...
		case 'save_edit':
			...
			$found_numerocompro=$ventas->search_cod($numerocomprobante);
			...
			if ($found_numerocompro==0) { 
				...
				if (($tipo_aux == "Boleta") || ($tipo_aux == "Factura") || ($tipo_aux == "Ticket")) {
					...
					$response = $ventas->create($codproveedor,$tipo_comprobante,$seriecomprobante,
		...
		case 'disable':
			...
			$response = $ventas->disable($codingreso);
			echo $response ? "<div class='alert alert-info alert-dismissable'>
		...
		case 'mostrar':
			$response = $ventas->mostrar($codingreso);
		...
		case 'listarDetalleIngresos':
			...
			$response = $ventas->listarDetalleIngresos($codingreso);
			...
			while ($item = $response->fetch_object()) {
				echo '<tr class="filas">
						...
						<td>'.number_format(($item->PrecioVenta*$item->Cantidad),2).'</td>
					</tr>';
					$tota_auxiliar = $tota_auxiliar+(($item->PrecioVenta)*($item->Cantidad));
			}
			/**
		...
		case 'showAll':
			$response = $ventas->showAll();
			$data = Array();
			$no=1;
			while ($item=$response->fetch_object()) {
				$data[]=array(
					'0'=>$item->Fecha,
					'1'=>$item->TipoComprobante,
					'2'=>$item->SerieComprobante,
					'3'=>$item->NumeroComprobante,
					'4'=>number_format($item->TotalVenta,2),
					...
					'8'=>($item->Estado) ? 
						'<div style="margin:0 auto;margin-left:auto;margin-right:auto;text-align:center;">
							...
							onclick="disable('.$item->IDVenta.')"><i class="fa fa-power-off"></i>
							</button>' .
							'<button class="btn btn-info btn-xs" data-toggle="tooltip" data-placement="left" 
								title="Detalles" onclick="mostrar('.$item->IDVenta.')"><i class="fa fa-eye"></i>
							</button>
							</div>'
							:
							''
			...
			while ($item=$response->fetch_object()) {
				$data[]=array(
					...
					'5'=>'<div style="margin:0 auto;margin-left:auto;margin-right:auto;text-align:center;">
							...
							title="Agregar" onclick="agregardetallesingreso('.$item->CodArticulo.',\''.$item->NombreArticulo.'\','.$item->Precio.')"><i class="fa fa-plus"></i>
							</button>
						  </div>'
